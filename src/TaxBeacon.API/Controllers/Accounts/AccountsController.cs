...
public class AccountsController: BaseController
{
    ...
    /// <param name="accountId">Account id</param>
    /// <param name="cancellationToken"></param>
    /// <response code="200">Returns client referrals</response>
    /// <response code="401">User is unauthorized</response>
    /// <response code="403">The user does not have the required permission</response>
    /// <returns>Get client referrals</returns>
    [HasPermissions(Common.Permissions.Clients.Read, Common.Permissions.Accounts.Read)]
    [HttpGet("{accountId:guid}/client-referrals", Name = "GetClientReferrals")]
    [ProducesResponseType(typeof(AccountDetailsResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<IActionResult> GetClientReferralsAsync([FromRoute] Guid accountId,
        CancellationToken cancellationToken)
    {
    }
}

...
public sealed partial class AccountsServiceTests
{
    ...
    public async Task ExportReferralProspectsAsync_ReturnsReferralProspectExportDto(FileType fileType)
    {
        // Arrange
        ...
        // Act
        await _accountService.ExportReferralProspectsAsync(fileType, default);

        // Assert
        if (fileType == FileType.Csv)
        {
            _csvMock.Verify(x => x.Convert(It.IsAny<List<ReferralProspectExportDto>>()), Times.Once());
        }
        else if (fileType == FileType.Xlsx)
        {
            _xlsxMock.Verify(x => x.Convert(It.IsAny<List<ReferralProspectExportDto>>()), Times.Once());
        }
        else
        {

...
public partial class ContactServiceTests
{
    ...
    public async Task GetContactDetailsAsync_ContactExists_ReturnsContact()
    {
        ...
        var expectedResult = await _dbContext.AccountContacts.ProjectToType<AccountContact>().FirstAsync();

        // Act
        var getContactResult = await _contactService.GetContactDetailsAsync(contact.Id);
    ...
    [Fact]
    public async Task GetContactDetailsAsync_ContactDoesntExist_ReturnsNotFound()
    {
        // Arrange
        var tenant = TestData.TenantFaker.Generate();
        var account = TestData.AccountFaker
        ...
        // Act
        var getContactResult = await _contactService.GetContactDetailsAsync(Guid.NewGuid());
